...
<div class="zo-grid-wraper <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>">
    ...
    <?php endif;?>
    <div class="row zo-grid zo-grid-blog-style2 zo-grid-boxed-layout <?php echo esc_attr($atts['grid_class']);?>">
        <?php
        $posts = $atts['posts'];
        ...
        while($posts->have_posts()){
            ...
            ?>
            <div class="<?php echo esc_attr($atts['item_class']);?> zo-grid-boxed-item" data-groups='[<?php echo implode(',', $groups);?>]'>
                <?php get_template_part('single-templates/grid-boxed/content',get_post_format()); ?>
            </div>
            <?php
        }
        ?>
