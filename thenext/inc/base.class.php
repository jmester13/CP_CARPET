...
class ZO_Base
{
    ...
    public static function getPageTitle(){
        ...
        if (!is_archive()){
            ...
            /* search */
            elseif (is_search()):
                printf( __( 'Search Results for: %s', 'thenext' ), '<span>' . get_search_query() . '</span>' );
            /* 404 */ 
            elseif (is_404()):
                _e( '404', 'thenext');
			/* Blog */ 
            ...
            /* author. */
            elseif ( is_author() ) :
                printf( __( 'Author: %s', 'thenext' ), '<span class="vcard">' . get_the_author() . '</span>' );
            /* date */
            elseif ( is_day() ) :
                printf( __( 'Day: %s', 'thenext' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
                printf( __( 'Month: %s', 'thenext' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'thenext' ) ) . '</span>' );
            elseif ( is_year() ) :
                printf( __( 'Year: %s', 'thenext' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'thenext' ) ) . '</span>' );
            /* post format */
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
                _e( 'Asides', 'thenext' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
                _e( 'Galleries', 'thenext');
    ...
    public static function getBreadCrumb($separator = '') {
        ...
        /* search */
        if( is_search() ){ echo '<li>'.__("Search", 'thenext').'</li>'; }
        /* date */
        if( is_year() ){ echo '<li>'.get_the_time('Y').'</li>'; }
        /* 404 */
        if( is_404() ) {
            echo '<li>'.__("404 - Page not Found", 'thenext').'</li>';
        }
        /* archive */
