...
class WC_Widget_Cart_Search extends WP_Widget {
    public function __construct() {
        parent::__construct(
            ...
            __('Cart & Search', 'thenext'), // Name
            array('description' => __("Display the user's Cart and Search form in the sidebar.", 'thenext'),) // Args
        );
    ...
    function widget($args, $instance) {
        ...
        <div class="widget_cart_search_wrap">
            ...
            <div class="shopping_cart_dropdown">
                <div class="shopping_cart_dropdown_inner">
                    ...
                    <ul class="<?php echo implode(' ', $list_class); ?>">
                        ...
                        <?php else : ?>
                            ...
                            <li class="cart-list clearfix"><?php _e( 'No products in the cart.', 'thenext' ); ?></li>
                ...
                <a href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>" class="btn btn-primary left wc-forward"><?php _e( 'Cart', 'thenext' ); ?></a>
                <span class="total right"><?php _e( 'Total', 'thenext' ); ?>:<span><?php echo $woocommerce->cart->get_cart_subtotal(); ?></span></span>
            ...
            <div class="widget_searchform_content">
                <form role="search" method="get" action="<?php echo esc_url( home_url( '/'  ) );?>">
                    ...
                    <input type="submit" value="<?php echo esc_attr__( 'Search', 'thenext' )?>" />
					<?php if($woocommerce):?>
    ...
    function form( $instance ) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e( 'Title:', 'thenext' ); ?></label>
            <input id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
        </p>
		<p>
            <label for="<?php echo esc_url($this->get_field_id('show_cart')); ?>"><?php _e( 'Show Cart:', 'thenext' ); ?></label>
			<select id="<?php echo $this->get_field_id('show_cart'); ?>" class="widefat" name="<?php echo $this->get_field_name('show_cart'); ?>">
				<option value="0" <?php selected($show_cart,0); ?>><?php echo __('No','thenext'); ?></option>
				<option value="1" <?php selected($show_cart,1); ?>><?php echo __('Yes','thenext'); ?></option>
			</select>
        </p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('show_search')); ?>"><?php _e( 'Show Search:', 'thenext' ); ?></label>
			<select id="<?php echo $this->get_field_id('show_search'); ?>" class="widefat" name="<?php echo $this->get_field_name('show_search'); ?>">
				<option value="0" <?php selected($show_search,0); ?>><?php echo __('No','thenext'); ?></option>
				<option value="1" <?php selected($show_search,1); ?>><?php echo __('Yes','thenext'); ?></option>
			</select>
        </p>
    <?php
...
if(!function_exists('woocommerce_header_add_to_cart_content')){
    ...
    <div class="shopping_cart_dropdown">
        ...
        <?php if ( sizeof( $woocommerce->cart->get_cart() ) <= 0 ) : ?>
        <?php endif; ?>
        <a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class="btn btn-primary left wc-forward"><?php _e( 'Cart', 'thenext' ); ?></a>
        <span class="total right"><?php _e( 'Total', 'thenext' ); ?>:<span><?php echo $woocommerce->cart->get_cart_subtotal(); ?></span></span>
        <?php if ( sizeof( $woocommerce->cart->get_cart() ) <= 0 ) : ?>
