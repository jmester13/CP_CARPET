...
function zo_setup() {
	/*
	 * Makes ZoTheme available for translation.
	 *
	 * Translations can be added to the /languages/ directory.
	 * If you're building a theme based on ZoTheme, use a find and replace
	...
	register_nav_menu( 'primary', __( 'Primary Menu', 'thenext' ) );
...
function zo_scripts_styles() {
	...
	global $smof_data, $wp_styles,$post;
	
	/** theme options. */
	if(isset($post->ID)){
		$zo_meta_data  =  json_decode(get_post_meta($post->ID, '_zo_meta_data', true));
		$menu_sticky =  (!empty($zo_meta_data->_zo_enable_header_fixed) && $zo_meta_data->_zo_enable_header_fixed == 1)  ?  $zo_meta_data->_zo_enable_header_fixed : $smof_data['menu_sticky'] ;
	}else{
		$menu_sticky = 0;
	}
	...
	/* Add Magnific Popup */
	wp_enqueue_script('zotheme-magnific-js', get_template_directory_uri() . '/assets/js/jquery.magnific-popup.min.js', array( 'jquery' ),'2.1.5', true);
	wp_enqueue_style('zotheme-magnific-css', get_template_directory_uri(). '/assets/css/magnific-popup.css');
	/* Add Zo-skew js */
	wp_enqueue_script('zoskewjs', get_template_directory_uri() . '/assets/js/zo.skew.js', array( 'jquery' ), '1.0.0', true);
	/*
	...
	wp_enqueue_style( 'twentytwelve-ie', get_template_directory_uri() . '/assets/css/ie.css', array( 'zotheme-style' ), '20121010' );
	$wp_styles->add_data( 'twentytwelve-ie', 'conditional', 'lt IE 9' );
	
	/* WooCommerce */
	if(class_exists('WooCommerce')){
	    wp_enqueue_style( 'thenext', get_template_directory_uri() . "/assets/css/woocommerce.css", array(), '1.0.0');
	}
...
function zo_widgets_init() {
	register_sidebar( array(
		'name' => __( 'Main Sidebar', 'thenext' ),
		'id' => 'sidebar-1',
		'description' => __( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'thenext' ),
	...
	register_sidebar( array(
		'name' => __( 'Header Top Left', 'thenext' ),
		'id' => 'sidebar-2',
		'description' => __( 'Appears when using the optional Header with a page set as Header top left', 'thenext' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="wg-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Header Top Right', 'thenext' ),
		'id' => 'sidebar-3',
		'description' => __( 'Appears when using the optional Header with a page set as Header top right', 'thenext' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="wg-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Header Top Right Home 4', 'thenext' ),
		'id' => 'sidebar-header-top-h4',
		'description' => __( 'Appears when using the optional Header with a page set as Header top right', 'thenext' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="wg-title">',
	...
	register_sidebar( array(
    	'name' => __( 'Menu Search', 'thenext' ),
    	'id' => 'sidebar-4',
    	'description' => __( 'Appears when using the optional Menu with a page set as Menu right', 'thenext' ),
    	'before_widget' => '<aside id="%1$s" class="widget %2$s">',
    	'after_widget' => '</aside>',
    	'before_title' => '<h3 class="wg-title">',
    	'after_title' => '</h3>',
	...
	register_sidebar( array(
    	'name' => __( 'Left Sidebar Services', 'thenext' ),
    	'id' => 'sidebar-5',
    	'description' => __( 'Appears when using the optional Footer with a page set as Footer Top 1', 'thenext' ),
    	'before_widget' => '<aside id="%1$s" class="widget %2$s">',
    	'after_widget' => '</aside>',
    	'before_title' => '<h3 class="wg-title">',
    	'after_title' => '</h3>',
	...
	register_sidebar( array(
    	'name' => __( 'Footer Top 1', 'thenext' ),
    	'id' => 'sidebar-6',
    	'description' => __( 'Appears when using the optional Footer with a page set as Footer Top 2', 'thenext' ),
    	'before_widget' => '<aside id="%1$s" class="widget %2$s">',
    	'after_widget' => '</aside>',
    	'before_title' => '<h3 class="wg-title">',
    	'after_title' => '</h3>',
	...
	register_sidebar( array(
    	'name' => __( 'Footer Top 2', 'thenext' ),
    	'id' => 'sidebar-7',
    	'description' => __( 'Appears when using the optional Footer with a page set as Footer Top 3', 'thenext' ),
	...
	register_sidebar( array(
    	'name' => __( 'Footer Top 3', 'thenext' ),
    	'id' => 'sidebar-8',
    	'description' => __( 'Appears when using the optional Footer with a page set as Footer Top 4', 'thenext' ),
    	'before_widget' => '<aside id="%1$s" class="widget %2$s">',
    	'after_widget' => '</aside>',
    	'before_title' => '<h3 class="wg-title">',
    	'after_title' => '</h3>',
	...
	register_sidebar( array(
    	'name' => __( 'Footer Bottom Left', 'thenext' ),
    	'id' => 'sidebar-9',
    	'description' => __( 'Appears when using the optional Footer Bottom with a page set as Footer Bottom left', 'thenext' ),
    	'before_widget' => '<aside id="%1$s" class="widget %2$s">',
    	'after_widget' => '</aside>',
    	'before_title' => '<h3 class="wg-title">',
    	'after_title' => '</h3>',
	...
	register_sidebar( array(
    	'name' => __( 'Footer Bottom Right', 'thenext' ),
    	'id' => 'sidebar-10',
    	'description' => __( 'Appears when using the optional Footer Bottom with a page set as Footer Bottom right', 'thenext' ),
    	'before_widget' => '<aside id="%1$s" class="widget %2$s">',
    	'after_widget' => '</aside>',
    	'before_title' => '<h3 class="wg-title">',
    	'after_title' => '</h3>',
    ...
    register_sidebar( array(
        'name' => __( 'Newsletter', 'thenext' ),
        'id' => 'sidebar-11',
        'description' => __( 'Subscribe to get the most out of Newsletter', 'thenext' ),
        'before_widget' => '<aside id="%1$s" class="widget %2$s">',
        'after_widget' => '</aside>',
        'before_title' => '<h3 class="wg-title">',
        'after_title' => '</h3>',
    ...
    register_sidebar( array(
        'name' => __( 'Instagram Sidebar', 'thenext' ),
        'id' => 'sidebar-13',
        'description' => __( 'Appears when using to show image from an account from Instagram', 'thenext' ),
        'before_widget' => '<aside id="%1$s" class="widget %2$s">',
        'after_widget' => '</aside>',
...
function zo_add_subtitle_field(){
    ...
    if(in_array($screen->id, array('post'))){
        ...
        /* html. */
        echo '<div class="subtitle"><input type="text" name="post_subtitle" value="'.esc_attr($value).'" id="subtitle" placeholder = "'.__('Subtitle', 'thenext').'" style="width: 100%;margin-top: 4px;"></div>';
    }
}
...
function zo_paging_nav($query = null) {
    ...
    $links = paginate_links( array(
            ...
            'prev_text' => __( '<i class="fa fa-angle-double-left"></i>', 'thenext' ),
            'next_text' => __( '<i class="fa fa-angle-double-right"></i>', 'thenext' ),
    ) );
...
<div class="comment-main">
    <div class="comment-header">
        <div class="comment-user">
            <?php printf( __( '<span class="comment-author">%s</span>', 'thenext' ), get_comment_author_link() ); ?>
        </div>
        <?php if ( $comment->comment_approved == '0' ) : ?>
            <em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.' , 'thenext'); ?></em>
        <?php endif; ?>
...
function searchfilter($query) {
    if ($query->is_search && !is_admin() ) {
        $query->set('post_type',array('post','team','portfolio','client','project','service'));
    }
	return $query;
}
